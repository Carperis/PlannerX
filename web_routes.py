...
GOOGLE_CLIENT_ID = (
    "697687543481-stsr0foi21nlt6abfc2cvls4266ofskv.apps.googleusercontent.com"
)
...
def reset_password_request():
    ...
    if request.method == "POST":
        msg.append("There is no account with that email. You must register first.")
        if form.validate_on_submit():
            user = User.query.filter_by(email=form.email.data).first()
            if user:
                if user.google:
...
def email_verification_request():
    ...
    return render_template(
        "email_verification_request.html", form=form, msg=msg, email=email
    )
...
def index():
    msg = [
        "WARNING:",
        "The current version of the website is for TEST ONLY!",
        "Any of your data might be DELETED at any time!",
        "Our team is not responsible for any loss of your data!",
    ]
...
def plan(planID):
    ...
    if wapi.allow_access(user, plan):
        if request.method == "POST":
            try:
                ...
                EarlyT = (
                    float(str(EarlyTime).split(":")[0])
                    + (float(str(EarlyTime).split(":")[1])) / 60
                )
                LateT = (
                    float(str(LateTime).split(":")[0])
                    + (float(str(LateTime).split(":")[1])) / 60
                )
                courses = ""
                classes = []
                ...
                if msg == []:
                    ...
                    if os.path.exists(path):
                        shutil.rmtree(path)
                    wapi.getPreference(str(user.id), str(plan.id), prefDict, semester)
            ...
            pass
        if plan:
            check1 = wapi.checkSubmitSuccess(user.id, plan.id, plan.semester)
            check2 = wapi.checkPlanSuccess(user.id, plan.id, plan.semester)
            check3 = wapi.checkRankSuccess(user.id, plan.id, plan.semester)
            controls = [check1, check2, check3]
            editDate = wapi.getEditDate(user.id, plan.id, plan.semester)
    ...
    guidance = {
        ...
        "AvgScore": [
            ...
            "Enter a average RateMyProfessor score (0~5) of your professors. When you rank your plans, the plans with higher average score will be ranked higher.",
        ],
        "EarlyTime": [
            "Preferred Starting Time",
            "Enter a time you prefer to start your first class in a day. When you rank your plans, the plans with later starting time will be ranked higher.",
        ],
        "LateTime": [
            "Preferred Ending Time",
            "Enter a time you prefer to end your last class in a day. When you rank your plans, the plans with earlier finishing time will be ranked higher.",
        ],
        "courses": [
            "Courses",
            "Select courses you want to take. You can type the course code or the course name to search for the course. Multiple courses selection is available.",
        ],
        "semester": [
            "Semester",
            "Choose a school year and a term of the semester you want to take courses in.",
        ],
        "getschedules": [
            "Get Schedules",
            "Click this button to get your schedules. You can get your schedules only after you submit your preferences. The schedules below are shown in default order.",
        ],
        "rankschedules": [
            "Rank Schedules",
            "Click this button to rank your schedules. You can rank your schedules only after you get your schedules. The schedules below are shown in ranked order. The first one should be your best schedule.",
        ],
        "deleteplan": [
            "Delete Plan",
...
def getPlans(planID):
    ...
    if result == 0:
        msg.append(
            "Sorry, no schedule is available. There might be some inevitable time conflicts in your courses. Please try to change your preferences and submit again."
        )
    else:
...
def rankPlans(planID):
    ...
    semester = plan.semester
    try:
        msg += wapi.rank_all_schedules(semester, userID, planID, ignoreSeats=True)
    except Exception as e:
...
def showSchedule(planID, n):
    ...
    try:
        ...
        validSche = wapi.create_schedule_pic(semester, userID, planID, planName)
        if not validSche:
            return jsonify(num=plan.planNum + 1, details=details)

        details = wapi.getScheduleDetails(userID, planID, semester, planName)
        plan.planNum = newNum
        db.session.commit()
